...
- **Embedding space:** Similarity is computed in the embedding space defined by the baseline model (e.g., PCA space for `lpm_selftrained`, or scGPT embedding space for `lpm_scgptGeneEmb`)
- **No lookahead bias:** 
  - For training-data-based embeddings: PCA is fit on training data only (`pca.fit_transform(train_data)`), then test data is projected into that space (`pca.transform(test_data)`)
  ...
  - This ensures no information leakage: similarity is computed in a space learned exclusively from training data
